function buildArff(signalName,No,N)
%wqrs('100s') -->considerar usar otro anotador qrs
%%ecgpuwave('100s','test','wqrs');

%Lee la señal
[tm,ecg] = rdsamp(signalName);
%Filtra la señal
ecgf = wden(ecg(:,1),'heursure','s','one',2,'db4');
%Generar archivo con señal filtrada
%mat2wfdb(xd,'100s'); -->cambiar nombre

%Genera anotaciones picos, inicio y fin de P,QRS,T
ecgpuwave(signalName,'test',1,10000);

pwaves=rdann(signalName,'test',[],[],[],'p');
twaves=rdann(signalName,'test',[],[],[],'t');
nwaves=rdann(signalName,'test',[],[],[],'N');
[onwaves,type,subtype,chan,numon,comments]=rdann('223','test',[],[],[],'(');
[offwaves,type,subtype,chan,numoff,comments]=rdann('223','test',[],[],[],')');
%Grafica
