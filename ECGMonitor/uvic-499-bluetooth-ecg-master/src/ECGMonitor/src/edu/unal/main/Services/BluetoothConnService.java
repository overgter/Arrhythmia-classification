package edu.unal.main.Services;
...
import edu.unal.main.View.ECGChartActivity;
...
public class BluetoothConnService extends Service {
	...
	public static LinkedBlockingQueue<Double> bluetoothQueueForSaving = new LinkedBlockingQueue<Double>();
	public static LinkedBlockingQueue<Double> bluetoothQueueForUI = new LinkedBlockingQueue<Double>();
	...
	class BluetoothReadThread extends Thread {
		...
		public void run() {
			...
			char c;
			int waitCount=0;
            //TODO:  Eventually we just need to replace for this
           /* try{
            XmlParser parser = new XmlParser();
             EcgObject ecgdata = parser.parse(iStream);
            //EcgObject ecgdata = parser.parse(getAssets().open("ecg.xml"));
